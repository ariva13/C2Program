    ...
    class Packet
    {
        ...
        public byte[] ExtensionBytes
        {
            get
            {
                ...
                result.AddRange(BitConverter.GetBytes(ReverseUnsignedShort(m_ExtensionFlags)));

                result.AddRange(BitConverter.GetBytes(ReverseUnsignedShort(m_ExtensionLength)));
            ...
            set
            {
                ...
                else
                {
                    ...
                    m_ExtensionFlags = ReverseUnsignedShort(BitConverter.ToUInt16(value, 0));

                    //Take the length in 32 bit words
                    ExtensionLength = (ushort)(ReverseUnsignedShort(BitConverter.ToUInt16(value, 2)));
        ...
        public Packet(int payloadSize = MaxPayloadSize, int version = 2) { m_Payload = new byte[payloadSize]; Created = DateTime.UtcNow; Version = version; }

        public Packet(byte[] payload, int payloadLength, ushort sequence, uint sourceId, uint timestamp)
            : this(payloadLength, 2)
        {
            //copy payload into m_Payload
            ...
            //m_TimeStamp = (uint)DateTimeToNptTimestamp(DateTime.UtcNow);
      ...
      /*        public Packet(ArraySegment<byte> packetReference, byte? channel = null)
                {
        ...
        public Packet(byte[] packet, int offset = 0)
...
*/
        ...
        public byte[] ToBytes(uint? ssrc = null)
        {
            ...
            result.AddRange(BitConverter.GetBytes(ReverseUnsignedShort(m_SequenceNumber)));

            //Add the Timestamp
            result.AddRange(BitConverter.GetBytes(ReverseUnsignedInt(m_TimeStamp)));

            //Add the SynchonrizationSourceIdentifier
            result.AddRange(BitConverter.GetBytes(ReverseUnsignedInt((ssrc ?? m_Ssrc))));

            if (ContributingSourceCount > 0)
            {
                //Loop the sources and add them to the header
                m_ContributingSources.ForEach(cs => result.AddRange(BitConverter.GetBytes(ReverseUnsignedInt(cs))));
            }
        ...
        public static uint DateTimeToNptTimestampBase(DateTime value) { return (uint)((DateTimeToNptTimestamp(value)) & 0xFFFFFFFF); }
